...
public class LevelBuilder : MonoBehaviour
{
    ...
    public void BuildLevel()
    {
        ...
        for (int i = 0; i < notes.Length; i++)
        {
            ...
            float[] notesDuration;
            float positionX = notes[i].time * playerSpeed; // Position in X axis that the enemy will be placed
            switch (notes[i].name)
            {
                case "C0":
                    ...
                    enemyBehaviour.nextPositions = new float[1];
                    enemyBehaviour.nextPositions[0] = notes[i + 1].time * playerSpeed; // New enemy position after hit 

                    enemyBehaviour.notesDuration = new float[2];
                    notesDuration = enemyBehaviour.notesDuration;
                ...
                case "D0":
                    ...
                    enemyBehaviour = enemy.GetComponent<EnemyBehaviour>();
                    enemyBehaviour.notesDuration = new float[1];
                    enemyBehaviour.notesDuration[0] = notes[i].duration;
                    break;
                case "D1":
